...
const AdminNotepad = () => {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        const nextPage = pageResponse?.data?.page || createEmptyPage(1, nextMetadata.minLines);
    ...
    try {
      await requestSecretKeyChallenge({
        reason: user?.hasSecretKey
          ? 'Verify your secret key for Darya page updates.'
          : 'Create your secret key first with email OTP for Darya page updates.',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nextPage = response?.data?.page || createEmptyPage(selectedPageNumber, lineCount);
      setCurrentPage(nextPage);
      setTitle(String(nextPage.title || ''));
      setContent(String(nextPage.content || ''));
        ...
        <div className="admin-page__shell admin-notepad">
          <header className="admin-page__header admin-notepad__header">
            <div className="admin-page__title-wrap">
              <span className="admin-page__eyebrow">Hidden Admin Label</span>
              <h1 className="admin-page__title">{metadata.heading || EMPTY_METADATA.heading}</h1>
              <p className="admin-page__subtitle">
                Private notepad pages for <strong>{user?.email || 'this admin account'}</strong>. Each admin gets 20
                separate pages, and the same previous text appears again after login.
              </p>
            </div>

            <div className="admin-page__toolbar">
              <Link to="/admin/dashboard" className="admin-btn admin-btn--muted admin-notepad__back-link">
                <ArrowLeft size={16} />
            ...
            <div className="admin-notepad__meta-grid">
              <article className="admin-notepad__meta-card">
                ...
                <p className="admin-notepad__meta-copy">
                  Total {metadata.totalPages || EMPTY_METADATA.totalPages} pages for this admin account.
                </p>
              </article>
              ...
              <article className="admin-notepad__meta-card">
                ...
                <strong className="admin-notepad__meta-value">{user?.email || 'No admin email'}</strong>
                <p className="admin-notepad__meta-copy">
                  This data is private to the current login email only.
                </p>
              </article>
            </div>
              ...
              <article className="admin-notepad__editor-card">
                ...
                <label className="admin-notepad__field">
                  ...
                  <textarea
                    ...
                    placeholder="Write your admin notes here. Previous text will appear here again after login."
                    disabled={isLoadingPage || isSavingPage}
                  />
                </label>
              </article>
